...
function M.open(opts)
  ...
  local function confirm()
    ...
    local cb = on_done; on_done = nil
    do_close()
    if cb then pcall(cb, result, nil) end
  end

  local function cancel()
    local cb = on_done; on_done = nil
    do_close()
    if cb then pcall(cb, nil, nil) end
  end
  ...
  vim.keymap.set("n", "<ScrollWheelDown>", function() move(1) end, km)
  vim.keymap.set("n", "<ScrollWheelUp>", function() move(-1) end, km)
