...
local api = require("lib.jira.api")
local icons = require("lib.jira.icons")
local util = require("lib.jira.util")
...
local function do_close(restore)
  if _state.closing then return end
  _state.closing = true
...
function M.open(key, current_status, on_done, opts)
  ...
  api.get_transitions(key, function(data, err)
    ...
    _state.autocmd_id = vim.api.nvim_create_autocmd("WinLeave", {
      buffer = buf,
      callback = function()
        vim.schedule(function() do_close(false) end)
      end,
    })
    ...
    local function confirm()
      ...
      api.do_transition(key, t.id, function(_, derr)
        if derr then
          ...
          if on_done then pcall(on_done, nil, derr) end
          return
        end
        vim.notify(string.format("xray: %s → %s", key, to_name), vim.log.levels.INFO)
        if on_done then pcall(on_done, to_name, nil) end
      end)
    end
    ...
    vim.keymap.set("n", "<ScrollWheelDown>", function() move(1) end, opts)
    vim.keymap.set("n", "<ScrollWheelUp>", function() move(-1) end, opts)
    vim.keymap.set("n", "<CR>", confirm, opts)
    vim.keymap.set("n", "<LeftMouse>", function()
      local pos = vim.fn.getmousepos()
      if not pos or pos.winid ~= win then return end
