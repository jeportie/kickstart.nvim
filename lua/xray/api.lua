...
local jira = require("lib.jira.api")
...
function M.get_issues_by_keys(keys, callback)
  ...
  if not creds then
    vim.schedule(function() callback(nil, "xray: missing credentials") end)
    return
  end
...
function M.build_jql(project_key, query)
  ...
  if numeric then
    return string.format(
      'project = %s AND issuekey = "%s-%s" ORDER BY updated DESC',
      project_key, project_key, numeric
    )
  end
