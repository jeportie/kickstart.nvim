...
local api = require("xray.api")
local cache = require("xray.cache")
...
local MULTI_FIELDS = {
  platforms    = { id = "customfield_10977", label = "Platforms",    options = PLATFORM_OPTIONS,     is_platform_like = true },
  automated_on = { id = "customfield_10975", label = "Automated on", options = AUTOMATED_ON_OPTIONS, is_platform_like = false, id_map = AUTOMATED_ON_ID },
}
...
function M.dispatch(r, opts)
  ...
  if r.field == "link" then
    ...
    if type(target) == "string" and target ~= "" then
      require("xray.hover").trigger(target, { focus = true })
    end
    return
  end

  if r.field == "status" then
    require("xray.pickers.status").open(key, r.current, function(new_status, err)
      if err or not new_status then return end
      cache.clear(key)
      if opts.on_status_change then pcall(opts.on_status_change, new_status) end
      if opts.on_refresh then pcall(opts.on_refresh) end
    end, { anchor = anchor })
    return
  end

  if r.field == "add_comment" then
    require("xray.pickers.comment").open(key, function(ok, err)
  ...
  if r.field == "assignee" then
    ...
    require("xray.pickers.assignee").open(key, current, function(result, err)
  ...
  if multi then
    require("xray.pickers.multi_select").open({
