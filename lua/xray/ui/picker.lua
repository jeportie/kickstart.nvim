...
local api_jira = require("xray.api")
local cache = require("xray.cache")
local config = require("xray.config")
local format = require("xray.format")
local icons = require("xray.icons")
local util = require("xray.util")
local insert_mod = require("xray.insert")
...
local function close_all()
  ...
  S.closed = true
  pcall(function() require("xray.help").close() end)
  if S.detail_buf then
    pcall(function() require("xray.editor").detach(S.detail_buf) end)
  end
...
local function setup_detail_keymaps(buf)
  ...
  require("xray.editor").attach(buf, S.detail_win, {}, {
    on_activate = function(r)
      local issue = selected_issue()
      if not issue then return end
      require("xray.edit").dispatch(r, {
        key = issue.key,
