...
local card = require("jira-board.ui.card")
...
local function build_band(band, band_idx, vcols, col_w, board_w, selected_card_key, collapsed)
  ...
  local toggle_action = function()
    require("jira-board.actions").toggle_epic(band.key)
  end
  -- 1. Epic header row
  ...
  for vidx, col in ipairs(vcols) do
    ...
    if #issues == 0 then
      ...
      for idx, issue in ipairs(issues) do
        ...
        local actions = {
          click = function()
            require("jira-board.ui.window").on_card_click(bi, vi, ci)
          end,
        }
        local status_actions = {
          click = function()
            ...
            vim.schedule(function()
              require("jira-board.actions").transition_selected()
            end)
          end,
        }
