...
local jira_api = require("lib.jira.api")
local jira_adf = require("lib.jira.adf")
local jira_util = require("lib.jira.util")
local hl = require("jira-board.ui.hl")
local icons = require("lib.jira.icons")
...
local function close()
  ...
  if #_stack > 0 then
    ...
    pcall(function() require("jira-board.ui.window").unlock_scroll() end)
  end
end
...
local function refresh_board_in_background()
  pcall(function()
    local win = require("jira-board.ui.window")
    if win.is_open() then require("jira-board.actions").refresh() end
  end)
end
...
local function trigger_transition()
  ...
  local status_picker = require("lib.jira.pickers.status")
...
local function trigger_assign_other()
  local my_state = _state
  if not (my_state and my_state.key) then return end
  local picker = require("lib.jira.pickers.assignee")
...
local function trigger_priority()
  ...
  require("jira-board.ui.priority_picker").open(my_state.key, cur, function(ok)
...
local function trigger_labels()
  ...
  require("jira-board.ui.labels_picker").open(my_state.key, labels, function(new)
...
local function trigger_description()
  ...
  require("jira-board.ui.text_editor").open({
    ...
    for _, rel in ipairs(order) do
      ...
      for _, t in ipairs(groups[rel]) do
        ...
        local link_actions = { click = function()
          local open = require("jira-board.ui.preview").open
      ...
      for _, r_cells in ipairs(md_rows) do
        ...
        if ticket_key then
          ...
          click = { click = function()
            require("jira-board.ui.preview").open({ key = child_key }, { stack = true })
          end }
        end
...
function M.open(issue, opts)
  ...
  pcall(function() require("jira-board.ui.window").lock_scroll() end)
  ...
  kmap("?", function() require("jira-board.ui.preview_help").toggle() end)
