...
local jutil = require("lib.jira.util")
local store = require("jira-board.store")
local kanban = require("jira-board.ui.kanban")
local hl = require("jira-board.ui.hl")
local cfg = require("jira-board.config")
...
local function close()
  ...
  M._state = {
    buf = nil, win = nil, dim_buf = nil, dim_win = nil,
    ns = nil, layout = nil, built = nil,
    cursor = { band = 1, col = 1, idx = 1 },
  }
end
...
function _G.JiraBoardCloseClick()
  require("jira-board.ui.window").close()
end
...
local CARD_H = require("jira-board.ui.card").CARD_HEIGHT
...
local function find_first_valid_cursor(built)
  ...
  for bi, b in ipairs(bands) do
    ...
    for ci = 1, built.ncols do
      ...
      if #issues > 0 then return { band = bi, col = ci, idx = 1 } end
    end
  end
...
local function valid_cursor(built, cur)
  ...
  if b.collapsed then return cur.idx == 0 end
  local issues = b.issues_by_col[cur.col] or {}
  return cur.idx >= 1 and cur.idx <= #issues
end

local function snap_cursor(built, cur)
  if valid_cursor(built, cur) then return cur end
  local bands = built and built.bands or {}
  if #bands == 0 then return { band = 1, col = 1, idx = 1 } end
  ...
  if b then
    ...
    for ci = 1, built.ncols do
      ...
      if #iss > 0 then return { band = cur.band, col = ci, idx = 1 } end
    end
  end

  local col = cur.col
  if col < 1 or col > built.ncols then col = 1 end
  ...
  for bi, bd in ipairs(bands) do
    ...
    if bd.collapsed then
      ...
      if #issues > 0 then this_idx = math.min(cur.idx, #issues) end
    end
    if this_idx then
...
function M.selected_issue()
  ...
  if not (built and built.bands and built.bands[cur.band]) then return nil end
  if cur.idx == 0 then return nil end
...
local function card_at_mouse()
  ...
  local s = M._state
  if not (s.win and s.built and s.layout) then return nil end
  if pos.winid ~= s.win then return nil end
  ...
  if not target_band then return nil end
  local b = target_band.band
  local cards_row_start = b.start_row + b.cards_offset
  if grel < cards_row_start then return nil end
  ...
  if vidx < 1 or vidx > s.built.ncols then return nil end
  local issues = b.issues_by_col[vidx] or {}
  if card_idx < 1 or card_idx > #issues then return nil end
  return target_band.idx, vidx, card_idx, issues[card_idx]
end

function M.card_at_mouse() return card_at_mouse() end
...
function M.move(dir)
  ...
  local cur = s.cursor
  local built = s.built
  if not built or not built.bands or #built.bands == 0 then return end
  local ncols = built.ncols
  local band = built.bands[cur.band]
  if not band then return end

  if dir == "right" or dir == "left" then
    if band.collapsed then return end
...
local function build_layout(built)
  return { { name = "grid", lines = function() return built.grid end } }
end
...
function M.move_cursor_to_selection()
  local s = M._state
  if not (s.win and api.nvim_win_is_valid(s.win)) then return end
  if not (s.built and s.layout) then return end
  local cur = s.cursor
  local built = s.built
  ...
  if row_1 > maxrow then row_1 = maxrow end
  if row_1 < 1 then row_1 = 1 end
...
local function set_keymaps(buf)
  ...
  map("q", close)
  map("<Esc>", close)
  map("<Tab>", function() M.move("right") end)
  map("<S-Tab>", function() M.move("left") end)
  map("l", function() M.move("right") end)
  map("h", function() M.move("left") end)
  map("j", function() M.move("down") end)
  map("k", function() M.move("up") end)
  map("<Down>", function() M.move("down") end)
  map("<Up>", function() M.move("up") end)
  map("<Right>", function() M.move("right") end)
  map("<Left>", function() M.move("left") end)
  map("<CR>", function() require("jira-board.actions").preview_selected() end)
  map("b", function() require("jira-board.actions").open_selected() end)
  map("y", function() require("jira-board.actions").yank_selected() end)
  map("R", function() require("jira-board.actions").refresh() end)
  map("A", function() require("jira-board.actions").toggle_assignee_filter() end)
  map("B", function() require("jira-board.actions").toggle_backlog() end)
  map("f", function() require("jira-board.ui.filter_picker").open() end)
  map("t", function() require("jira-board.actions").transition_selected() end)
  map("p", function() require("jira-board.actions").preview_selected() end)
  map("m", function() require("jira-board.actions").assign_selected_to_me() end)
  map("a", function() require("jira-board.actions").assign_selected_to_other() end)
  map("u", function() require("jira-board.actions").unassign_selected() end)
  map("z", function() require("jira-board.actions").toggle_epic_at_cursor() end)
  map("Z", function() require("jira-board.actions").toggle_all_epics() end)
  map("?", function() require("jira-board.actions").show_help() end)
  map("<C-l>", function() M.rerender() end)
  map("<RightMouse>", function() require("jira-board.ui.menu").open_at_mouse() end)
  ...
  map("<2-LeftMouse>", function()
    ...
    require("jira-board.actions").preview_selected()
  end)
end
...
local function open_main(board_w, total_h)
  ...
  vim.wo[win].winhighlight =
    "Normal:JiraBoardNormal,FloatBorder:JiraBoardBorder," ..
...
function M.open()
  ...
  api.nvim_create_autocmd("BufWipeout", {
    buffer = buf,
    callback = function() volt_shim.cleanup_buf(buf) end,
  })
